...
Logger::Logger(const std::string& name)
    ...
    m_formatter.reset(new LogFormatter("%d{%Y-%m-%d %H:%M:%S}%T%t%T[%p]%T[%c]%T%f:%l%T%m%n"));
        // m_formatter.reset(new LogFormatter("%d{%Y-%m-%d %H:%M:%S}%T%t%T[%p]%T[%c]%T%m%n"));
}
...
FileLogAppender::FileLogAppender(const char* path, 
                const char* suffix) {
    time_t timer = time(nullptr);
    struct tm *sysTime = localtime(&timer);
    struct tm t = *sysTime;
    char fileName[128] = {0};
    snprintf(fileName, 126, "%s/%04d_%02d_%02d%s", 
...
LoggerManager::LoggerManager(bool flag) {
