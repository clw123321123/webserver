...
static kong::Logger::ptr g_logger = KONG_LOG_NAME("httprequest");
...
bool HttpRequest::parse(Buffer& buff) {
    ...
    KONG_LOG_DEBUG(g_logger) << method_.c_str() << path_.c_str() << version_.c_str();
    return true;
}
...
bool HttpRequest::ParseRequestLine_(const string& line) {
    ...
    KONG_LOG_ERROR(g_logger) << "RequestLine Error";
    return false;
}
...
void HttpRequest::ParseBody_(const string& line) {
    ...
    ParsePost_();
    state_ = FINISH;
    KONG_LOG_DEBUG(g_logger) << "Body:" << line.c_str() << "Len:" << line.size();
}
...
void HttpRequest::ParsePost_() {
    if(method_ == "POST" && header_["Content-Type"] == "application/x-www-form-urlencoded") {
        ...
        if(DEFAULT_HTML_TAG.count(path_)) {
            ...
            KONG_LOG_DEBUG(g_logger) << "Tag:" << tag;
            if(tag == 0 || tag == 1) {
                bool isLogin = (tag == 1);
...
void HttpRequest::ParseFromUrlencoded_() {
    ...
    for(; i < n; i++) {
        ...
        case '&':
            ...
            j = i + 1;
            post_[key] = value;
            KONG_LOG_DEBUG(g_logger) << key.c_str() << "="<< value.c_str();
            break;
        default:
            break;
        }
...
bool HttpRequest::UserVerify(const string &name, const string &pwd, bool isLogin) {
    ...
    KONG_LOG_INFO(g_logger) << "Verify name:" << name.c_str() << "  " << "pwd" <<pwd.c_str();
    MYSQL* sql;
