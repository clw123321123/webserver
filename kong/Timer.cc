...
bool Timer::siftdown_(size_t index, size_t n) {
    KONG_ASSERT(index >= 0 && index < heap_.size());
    KONG_ASSERT(n >= 0 && n <= heap_.size());
    size_t i = index;
    size_t j = i * 2 + 1;
    while(j < n) {
...
void Timer::del_(size_t index) {
    /* 删除指定位置的结点 */
    KONG_ASSERT(!heap_.empty() && index >= 0 && index < heap_.size());
    /* 将要删除的结点换到队尾，然后调整堆 */
    size_t i = index;
...
void Timer::adjust(int id, int timeout) {
    /* 调整指定id的结点 */
    KONG_ASSERT(!heap_.empty() && ref_.count(id) > 0);
